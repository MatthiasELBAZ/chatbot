from Self_Coach_It_Classes_v2 import *
...
def main():
    ...
    llm_name_model = input('LLM name model [openai_3.5, openai_4, openai_4o, anthropic_opus, anthropic_sonnet, mistral_large, mistral_8x22B]: ')
    embedding_name_model = input('Embedding name model [openai, mistralai]- anthropic not have emdedding model: ')
    temperature = input('LLM temperature ([0,1]): ')
    temperature = float(temperature)
    decay_rate = input('Time Retriever decay rate ([0,1]): ')
    decay_rate = float(decay_rate)
    k = input('Time Retriever k (odd integer): ')
    k = int(k)
    buffer_num_ongo_messages = input('buffer num ongoing messages (even integer): ')
    ...
    Load_Models = LoadModels(llm_api_key, embedding_api_key, llm_name_model, embedding_name_model, temperature=temperature, max_tokens=4096)
    ...
    # make it as a chain
    Pinecone_Time_Retiever.make_it_chain()

    # time retriever
    pinecone_time_retriever_chain = Pinecone_Time_Retiever.time_retriever_chain
    ...
    formulaire_retriever_chain = Formulaire_Retriever.formulaire_retriever
    print('Formulaire Retriever')
    print()

    # chatbot
    Retrieval_Document_Chain_Memory = RetrievalDocumentChainMemory(
        llm, 
        pinecone_time_retriever_chain,
        formulaire_retriever_chain,
        user_profile, 
        user_id, 
        session_id, 
        store_history, 
