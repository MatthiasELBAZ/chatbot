...
class UserProfile():
    ...
    def make_summarizer(self) -> None:
        ...
        summary_prompt = ChatPromptTemplate.from_messages([("system", self.prompts.journal_user_summary_system_prompt), MessagesPlaceholder(variable_name="messages")])
    ...
    def get_user_summary(self) -> None:
        # invoke summarizer
        user_summary = self.summarizer.invoke({"context": self.formulaire_docs, "messages": [HumanMessage(content=self.prompts.user_summary_user_prompt)]})
    ...
    def get_journal_summary(self) -> None:
        # invoke summarizer
        journal_summary = self.summarizer.invoke({"context": self.journal_docs, "messages": [HumanMessage(content=self.prompts.journal_summary_user_prompt)]})
    ...
    def get_user_profile(self, user_summary: str, journal_summary: str) -> None:
        ...
        user_profiler_prompt = ChatPromptTemplate.from_messages([("system", self.prompts.user_profile_system_prompt), MessagesPlaceholder(variable_name="messages")])
        ...
        user_profile = user_profiler_chain.invoke(
            {
                ...
                "messages": [HumanMessage(content=self.prompts.user_profile_user_prompt)]
            }).content
...
class CallBacker(BaseCallbackHandler):
    def __init__(self, llm):
        ...
        elif 'gpt-4o' in self.llm_name_model:
...
class RetrievalDocumentChainMemory(UserSessionStoreHistory):
    ...
    def summarize_ongo_messages(self, chain_input: Any) -> bool:
        ...
        if len(stored_messages)<=self.buffer_num_ongo_messages:
            ...
            summarization_prompt = ChatPromptTemplate.from_messages([MessagesPlaceholder(variable_name="chat_history"), ("user", self.prompts.ongo_summary_user_prompt)])
    ...
    def get_retrieval_document_chain_with_message_history(self) -> None:
        ...
        document_chain = coach_chatbot_prompt | self.llm
    ...
    def run_chat(self, input: str) -> [Dict, Any]:
        ...
        result = self.retrieval_document_chain_with_message_history.invoke(
            {
                ...
                "rag_journal_context": (self.time_retriever).invoke(input),
        ...
        if result['messages_summarized']==True:
            ...
            self.store_history[self.user_id][self.session_id]['ongo'].add_message(self.summary_message)
        ...
        self.store_history[self.user_id][self.session_id]['full'].add_message(AIMessage(result['answer'].content))
...
class NewJournal():
    ...
    def get_new_journal(self) -> None:
        ...
        summarization_prompt = ChatPromptTemplate.from_messages([MessagesPlaceholder(variable_name="chat_history"), ("user", self.prompts.full_chat_to_journal_user_prompt)])
