...
class LoadModels():
    def __init__(self, llm_name_model: str, temperature: float) -> None:
        self.llm_name_model = llm_name_model
        self.temperature = temperature
        self.max_tokens = 4096

    def select_api_key(self) -> str:
        llm_name_model = self.llm_name_model
        if 'gpt' in llm_name_model:
    ...
    def select_llm_model(self, api_key) -> Any:
        llm_name_model = self.llm_name_model
        if 'gpt' in llm_name_model:
    ...
    def select_embedding_model(self, api_key) -> Any:
        llm_name_model = self.llm_name_model
        api_key = os.getenv("OPENAI_API_KEY")
        if 'gpt' in llm_name_model:
...
class UserProfile():
    ...
    def user_profile_generation(self, llm, prompts, formulaire_docs,  journal_docs, user_summary) -> None:
        ...
        # make summarizer
        self.make_summarizer(llm, self.prompts)

        # get user summary
        logging.debug(f"Recupération du summary:")
        self.get_user_summary(formulaire_docs, self.prompts)
        ...
        if len(journal_docs) == 0:
            ...
            self.get_journal_summary(journal_docs, self.prompts)
    ...
    def from_csv(self, user_profile_csv_path: str) -> None:
        # load user profile from csv
        df = pd.read_csv(user_profile_csv_path)
        try:
            self.user_profile = df['profile'].values[-1]
        except:
            self.user_profile = None
...
class RetrievalDocumentChainMemory():
    ...
    def get_retrieval_document_chain_with_message_history(self, llm: Any, prompts, user_id: str, session_id: str, store_history: Dict, buffer_num_ongo_messages: int) -> None:
        ...
        self.prompts = prompts
        coach_chatbot_prompt = ChatPromptTemplate.from_messages([("system", self.prompts['system_prompt_coach']), MessagesPlaceholder("chat_history"), ("human", self.prompts['user_prompt_coach'])])
...
class UpdateStore():
    ...
    def update_store_user_profile_csv(self, user_profile_csv_path: str, user_profile: str, session_id: str) -> None:
        # load pandas user_profile.csv
        df = pd.read_csv(user_profile_csv_path)

        # add new line at date of today with the summary message
        r = pd.DataFrame({str(len(df)+1):{'date': get_today(), 'profile': user_profile}}).T
        r['session_id'] = session_id 
        df = pd.concat([df, r], ignore_index=True)
...
def initialization(user_directory, user_id, session_id, user_starting_date, namespace, index_name, coach_name, llm_name_model, 
                   temperature, decay_rate, k, buffer_num_ongo_messages, system_prompts, user_prompts, summary_prompts,
                   journal_file, formulaire_file, history_file, profile_file):
    ...
    logging.debug(f"LoadModels")
    Load_Models = LoadModels(llm_name_model, temperature)
    logging.debug(f"LLM API KEY:")
    llm_api_key = Load_Models.select_api_key()
    logging.debug(f"LLM Model:")
    llm = Load_Models.select_llm_model(llm_api_key)
    logging.debug(f"Embedding Model:")
    embedding = Load_Models.select_embedding_model(llm_api_key)
    ...
    # User Profile Initialization - User Summary - short cut user profile generation
    logging.debug(f"USERPROFILE:")
    User_Profile = UserProfile()
    user_profile_csv_path = user_directory+profile_file
    User_Profile.from_csv(user_profile_csv_path)
    ...
    initialization_dict = {
        ...
        'formulaire_file': formulaire_file,
...
def update_and_store_after_exit(initialization_dict):
    ...
    journal_csv_path = initialization_dict["user_directory"]+initialization_dict['journal_file']
    chat_history_csv_path = initialization_dict["user_directory"]+initialization_dict['history_file']
    user_profile_csv_path = initialization_dict["user_directory"]+initialization_dict['profile_file']
    ...
    # update user profile
    User_Profile.get_user_profile(llm, User_Profile.user_summary, new_journal, prompts)
    # update user profile csv
    Update_Store.update_store_user_profile_csv(user_profile_csv_path, User_Profile.user_profile, session_id)
    print("User Profile updated.")

    initialization_dict['User_Profile'] = User_Profile
    initialization_dict['Update_Store'] = Update_Store
    initialization_dict['New_Journal'] = New_Journal
...
def run_chat(input, initialization_dict, modify_llm_name_model):
    ...
    coach_name = initialization_dict['coach_name']
    llm_name_model = initialization_dict['llm_name_model']
    ...
    if user_profile is None:
        user_profile = User_Profile.user_summary
    ...
    logging.debug("#########################")
    logging.debug(f"{llm_name_model}")
    logging.debug(f"{modify_llm_name_model}")
    ...
    if modify_llm_name_model!=llm_name_model:
        ...
        # update the llm in the initialization_dict
        initialization_dict['llm_name_model'] = modify_llm_name_model
    ...
    if input.lower() == 'exit':
        ...
        update_dict = update_and_store_after_exit(initialization_dict)

        update_dict['result'] = results
        update_dict['callback'] = callback
        update_dict['Retrieval_Document_Chain_Memory'] = Retrieval_Document_Chain_Memory
...
def main():
    ...
    # read dictioanry from text file
    with open('initialize.txt', 'r') as file:
        initialize = eval(file.read())

    # get values from dictionary and assign to variables
    user_directory = initialize['user_directory']
    user_id = initialize['user_id']
    session_id = initialize['session_id']
    user_starting_date = initialize['user_starting_date']
    namespace = initialize['namespace']
    index_name = initialize['index_name']
    coach_name = initialize['coach_name']
    llm_name_model = initialize['llm_name_model']
    temperature = initialize['temperature']
    decay_rate = initialize['decay_rate']
    k = initialize['k']
    buffer_num_ongo_messages = initialize['buffer_num_ongo_messages']
    system_prompts = initialize['system_prompts']
    user_prompts = initialize['user_prompts']
    summary_prompts = initialize['summary_prompts']
    journal_file = initialize['journal_file']
    formulaire_file = initialize['formulaire_file']
    history_file = initialize['history_file']

    # initialization
    initialization_dict = initialization(user_directory, user_id, session_id, user_starting_date, namespace, index_name, coach_name, llm_name_model, 
                   temperature, decay_rate, k, buffer_num_ongo_messages, system_prompts, user_prompts, summary_prompts,
                   journal_file, formulaire_file, history_file)
    ...
    final_price = 0
    while True:
        # model selection -- need to be improved
        print('Model Name: ', llm_name_model)
        print()
        llm_name_select = input('Select Model Name: ')
        if len(llm_name_now) == 0:
            llm_name_now = 'same'
            print('Same Model: ', llm_name_model)
            print()
        else:
            llm_name_now = llm_name_select
            print('New Model Name: ', llm_name_now)
            print()

        # user input
        prompt = input('You: ')
        
        chat_dict = run_chat(prompt, initialization_dict, modify_llm_name_model=llm_name_now)
